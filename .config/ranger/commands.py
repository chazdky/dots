...
class fasd(Command):
    ...
    def _get_directories(*args):
        ...
        output = subprocess.check_output(
            ["fasd", "-dl", *args], universal_newlines=True
        )
...
class ag(Command):
    ...
    def tab(self):
        ...
        #   <= XPL: aliases expanded before parsing cmdline
        cmd = self.arg(0)
        flg = self.arg(1)
...
# Bash version; need to implent in python, and then I want to add a version to
# recognize the session id ie. tmux display-message -p "#S" as variable
class nvo(Command):
    pass
