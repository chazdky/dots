#!/usr/bin/env python3
# Created by Chaz Davis on 2020-04-19
...
def dmenu_cmd(
    num_lines, prompt="Networks", active_lines=None
    ...
    res += list(itertools.chain.from_iterabe(extras))
    res[1:1] = lines.split()
...
def choose_adapter(client):
    ...
    sel = (
        Popen(
            dmenu_cmd(len(devices), "CHOOSE ADAPTER:"), stdin=PIPE, stdout=PIPE, env=ENV
        )
...
def create_other_actions(client):
    ...
    actions = [
        Action("{} Wifi".format(wifi_action), toggle_wifi, not wifi_enabled),
        Action(
            "{} Networking".format(networking_action),
            toggle_networking,
...
def process_ap(nm_ap, is_active, adapter):
    ...
    if is_active:
        ...
        conns_cur = [
            ...
            and i.get_setting_wireless().get_mac_address()
...
def create_ap_actions(
    ...
    for nm_ap, name, sec in zip(aps, names, secs):
        ...
        wifi_chars = (
            conf.get("dmenu", "wifi_chars")
            if conf.has_option("dmenu", "wifi_chars")
            else False
        )
        if wifi_chars:
        ...
        compact = (
            conf.getboolean("dmenu", "compact")
            if conf.has_option("dmenu", "compact")
            else False
        )
        if compact:
            action_name = u"{}  {}  {}".format(name, sec, bars)
        else:
            action_name = u"{:<{}s}  {:<{}s}  {}".format(
...
def get_selection(eths, aps, vpns, wgs, gsms, blues, wwan, others):
    ...
    conf.read(expanduser("~/.config/networkmanager-dmenu/config.ini"))
    rofi_highlight = (
        conf.getboolean("dmenu", "rofi_highlight")
        if conf.has_option("dmenu", "rofi_highlight")
        else False
    )
    compact = (
    ...
    if rofi_highlight is False:
        action = [
            i
            for i in eths + aps + vpns + wgs + gsms + blues + wwan + others
            if (
    ...
    assert len(action) == 1, u"Selection was ambiguous: '{}'".format(str(sel.strip()))
    return action[0]
...
def get_passphrase():
    ...
    if pinentry:
        pin = ""
        out = Popen(pinentry, stdout=PIPE, stdin=PIPE).communicate(
        ...
        return pin
    return (
        Popen(dmenu_cmd(0, "Passphrase"), stdin=PIPE, stdout=PIPE)
        .communicate()[0]
        .decode(ENC)
    )
